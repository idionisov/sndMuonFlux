...
for run in runs:
    if run!=8329:
        ...
        data[run] = SndData(Run=run, InputDir="/eos/experiment/sndlhc/users/sii/2024", TopDir=str(run), Files=args.files)
    data[run].Print()

start_time = time()
count = 0
tree = {run: data[tt].Tree for tt in TTs}
nEntries = {run: tree[tt].GetEntries() for tt in TTs}
...
saveToRoot(h, fout="/eos/user/i/idioniso/mfout/trkMeanRes.root")
